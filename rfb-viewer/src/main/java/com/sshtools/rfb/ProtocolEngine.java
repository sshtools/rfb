...
public class ProtocolEngine implements KeyListener, MouseListener,
		MouseMotionListener, ClipboardOwner, Runnable {
	...
	public ProtocolEngine(RFBDisplay display, RFBTransport transport,
	...
	public Point getCurrentLocalCursorHotspot() {
		return currentLocalCursorHotspot;
	}
	...
	SecurityType determineAuthenticationRequired() throws IOException {
		...
		case RFBConstants.SCHEME_VNC_AUTHENTICATION:
			if (!securityTypeFactory.isAvailable(securityTypeCode)) {
				throw new IOException("Selected authentication scheme ("
						+ securityTypeCode + ") is not available.");
			}
			break;
		...
		default:
			throw new IOException(
					"The server reported an invalid authentication scheme! "
							+ "scheme=" + String.valueOf(securityTypeCode));
		}
	...
	void performClientInitialization() throws IOException {
		LOG.info("Initializing client (share desktop = "
				+ context.isShareDesktop() + ")");
	...
	void initializeProtocol() throws IOException {
		...
		for (RFBFS fs : new RFBFS[] { new TightVNCFS(this),
				new UltraVNCFS(this) }) {
			if (fs.isActive()) {
				fileSystem = fs;
	 ...
	 * @throws IOException
	 */
	public void requestFramebufferUpdate(int x, int y, int w, int h, boolean inc)
			throws IOException {
		if (LOG.isDebugEnabled()) {
			LOG.debug("Requesting frame buffer update for " + x + "," + y + ","
					+ w + "," + h + " = " + inc);
		}
	 ...
	 * @throws IOException
	 */
	void changePixelFormat(int bitsPerPixel, int colorDepth,
			boolean isBigEndian, boolean isTrueColor, int redMax, int greenMax,
	...
	BufferUpdate getFramebufferUpdateRect() throws IOException {
		BufferUpdate update = new BufferUpdate(in.readUnsignedShort(),
	...
	private void processProtocol() throws IOException,
		...
		try {
			...
			prompt.connected();
			prompt.resized(displayModel.getRfbWidth(),
					displayModel.getRfbHeight());
		...
		try {
			while (true) { // rely on the IOException to break out
				if (requestedFullUpdate) {
					requestFramebufferUpdate(0, 0, displayModel.getRfbWidth(),
				...
				case RFBConstants.SMSG_FRAMEBUFFER_UPDATE:
					...
					for (int i = 0; i < numUpdates; i++) {
						...
						try {
							...
							encoding = context.selectEncoding(rect
									.getEncoding());
							if (encoding != null) {
								if (LOG.isDebugEnabled()) {
								...
								encoding.processEncodedRect(display,
										rect.getX(), rect.getY(),
								...
								System.out.println("WARNING: Unknown encoding "
										+ rect.getEncoding());
							}
					...
					// }
					if (context.getDeferUpdateRequests() > 0
							&& in.available() == 0 && !cursorPosReceived) {
					...
					requestFramebufferUpdate(0, 0, displayModel.getRfbWidth(),
							displayModel.getRfbHeight(), !fullUpdateNeeded);
					break;
				...
				default:
					if (fileSystem == null
							|| (fileSystem != null && !fileSystem
									.handleReply(type))) {
						throw new IOException("Unknown RFB message type "
								+ type);
					}
					break;
				}
	...
	private void readColourMap() throws IOException {
		...
		for (int i = 0; i < colours; i++) {
			int r = displayModel.getColorDepth() == 16 ? in.readShort() : (in
					.readShort() >> 8) & 0x000000ff;
			int g = displayModel.getColorDepth() == 16 ? in.readShort() : (in
					.readShort() >> 8) & 0x000000ff;
			int b = displayModel.getColorDepth() == 16 ? in.readShort() : (in
					.readShort() >> 8) & 0x000000ff;
			LOG.info(String.format("Map %d to %s %s %s", i,
	...
	private void handleAuth(SecurityType securityType) throws IOException,
			RFBAuthenticationException {
	...
	private void sendAuthenticationError() throws RFBAuthenticationException,
			IOException {
	...
	public SecurityType negotiateType() throws IOException {
		...
		for (int i = 0; i < securityTypes; i++) {
			...
			if (securityTypeFactory.isAvailable(serverType)
					&& (validSubAuths == null || (validSubAuths != null && validSubAuths
							.contains(serverType)))) {
				LOG.info("Server and client both support authentication type "
						+ serverType);
				supportedServerTypes.add(serverType);
			} else {
				LOG.info("Server support authentications type " + serverType
						+ " but this client does not");
			}
		}
		newSecurityTypeCode = securityTypeFactory
				.selectScheme(supportedServerTypes);
	...
	public void sendCtrlAltDel() throws IOException {
		if (isInputEnabled()) {
			...
			KeyEvent ctrlAltDelEvent = new KeyEvent(
					display.getDisplayComponent(), KeyEvent.KEY_PRESSED, 0,
					modifiers, KeyEvent.VK_DELETE, KeyEvent.CHAR_UNDEFINED);
			postKeyboardEvent(ctrlAltDelEvent);
			ctrlAltDelEvent = new KeyEvent(display.getDisplayComponent(),
					KeyEvent.KEY_RELEASED, 0, modifiers, KeyEvent.VK_DELETE,
	 ...
	 * @throws IOException
	 */
	public synchronized void postPointerEvent(MouseEvent evt)
			throws IOException {
		...
		int x = (int) (evt.getX() / displayModel.getXscale())
				- displayModel.getImagex();
		int y = (int) (evt.getY() / displayModel.getYscale())
				- displayModel.getImagey();
		if (x < 0) {
			x = 0;
		}
		if (y < 0) {
	...
	private void sendPointerEvent(int modifiers, int x, int y)
			throws IOException {
		eventBufferPos = 0;
	...
	public synchronized void postKeyboardEvent(KeyEvent evt) throws IOException {
		...
		if (keyChar == KeyEvent.CHAR_UNDEFINED) {
			...
			case KeyEvent.VK_CONTROL:
				key = evt.getKeyLocation() == KeyEvent.KEY_LOCATION_RIGHT ? 0xffe4
						: 0xffe3;
				break;
			case KeyEvent.VK_SHIFT:
				key = evt.getKeyLocation() == KeyEvent.KEY_LOCATION_RIGHT ? 0xffe2
						: 0xffe1;
				break;
			case KeyEvent.VK_ALT:
				key = evt.getKeyLocation() == KeyEvent.KEY_LOCATION_RIGHT ? 0xffea
						: 0xffe9;
				break;
			case KeyEvent.VK_META:
				key = evt.getKeyLocation() == KeyEvent.KEY_LOCATION_RIGHT ? 0xffe8
						: 0xffe7;
				break;
			}
		}
			...
			if (display.handleKeyEvent(evt)) {
				if (!inputEnabled) {
					if ((evt.getKeyChar() == 'r' || evt.getKeyChar() == 'R')
						...
						try {
							requestFramebufferUpdate(0, 0,
									displayModel.getRfbWidth(),
									displayModel.getRfbHeight(), false);
						} catch (IOException e) {
							e.printStackTrace();
						}
		...
		if (isProcessingEvents()) {
			if (inputEnabled) {
				if (context.getMouseEventDelay() != 0
						&& (evt.getID() == MouseEvent.MOUSE_MOVED || evt
								.getID() == MouseEvent.MOUSE_DRAGGED)) {
					if (mouseEventDispatcher == null
							|| !mouseEventDispatcher.isAlive()) {
	...
	private void doMoveCursor(int x, int y) {
		Rectangle displayClip = new Rectangle(displayModel.getImagex(),
				displayModel.getImagey(), display.getDisplayComponent()
						.getSize().width - (displayModel.getImagex() * 2),
				display.getDisplayComponent().getSize().height
		...
		if (changeToLocal) {
			...
			updateCursor(
					(int) (((float) x - (float) displayModel.getImagex()) / displayModel
							.getXscale()),
					(int) (((float) y - (float) displayModel.getImagey()) / displayModel
							.getYscale()));
		}
	}
				...
				if (context.isLocalCursorDisplayed()) {
					if (displayModel.getCursor() != null) {
						setLocalCursor(displayModel.getCursor(),
								displayModel.getHotX(), displayModel.getHotY());
					} else {
						setLocalCursor(null, -1, -1);
					}
				} else {
	...
	public void startRFBProtocol() throws IOException,
			RFBAuthenticationException {
		if (context.isAdaptive()) {
			setInputStream(new ProtocolReader(
					monitor = new MonitorDataInputStream(
	...
	public void setOutputStream(OutputStream out) {
		this.out = out instanceof ProtocolWriter ? (ProtocolWriter) out
				: new ProtocolWriter(out);
	}

	private void setLocalCursorImpl(Image img, int hotX, int hotY) {
		if ((localCursorImage == null && img != null)
				|| (img == null && localCursorImage != null)
	...
	private void setCursorImage(Image img, int hotX, int hotY) throws Exception {
		...
		Method createCustomCursorMethod = toolkit.getClass().getMethod(
				"createCustomCursor",
				new Class[] { Image.class, Point.class, String.class });
		Cursor cursor = (Cursor) (createCustomCursorMethod.invoke(toolkit,
				new Object[] { img, hotspot, "none" }));
		...
		public void run() {
			while (true) {
				try {
					...
					if (lastEvent != null
							&& events > context.getMouseEventThreshold()) {
						try {
							postPointerEvent(lastEvent);
						} catch (IOException ioe) {
							break;
	...
	public synchronized void startRecording(OutputStream stream)
			throws IOException {
