    ...
    public boolean process(RFBClient rfbClient) throws AuthenticationException {
        try {
            ...
            rfbClient.setInput(new ProtocolReader(new FilterInputStream(sslSocket.getInputStream()) {
                /*
                 * TODO This filter stream is a work around for the fact the SSL input streams
                 * ALWAYS return ZERO on available(). Because we only ever want
                 * to know if there is SOME data (>0), then querying the
                 * underlying stream should be enough. The long term solution is
                 * to replace the I/O streams with NIO channels
                 * 
                 * http://stackoverflow.com/questions/26320624/how-to-tell-if-java-sslsocket-has-data-available
                 */
                @Override
