...
public class WindowOutlineDisplayDriver extends FilteredDisplayDriver {
	...
	public BufferedImage grabArea(Rectangle area) {
		synchronized (lock) {
			...
			if (undraw) {
				...
				return bim;
				// return backingStore.getSubimage(area.x, area.y, area.width,
				// area.height);
			} else {
				BufferedImage bim = new BufferedImage(area.width, area.height, backingStore.getType());
				Graphics2D createGraphics = bim.createGraphics();
	...
	protected void fireRectangularDamage(String name, Rectangle rectangle, boolean undraw) {
		...
		fireDamageEvent(name, new Rectangle(rectangle.x, rectangle.y, rectangle.width, 1), -1);
		drawFrame.set(Border.SOUTH);
		fireDamageEvent(name, new Rectangle(rectangle.x, rectangle.y + rectangle.height - 1, rectangle.width, 1), -1);
		drawFrame.set(Border.WEST);
		fireDamageEvent(name, new Rectangle(rectangle.x, rectangle.y + 1, 1, rectangle.height - 2), -1);
		drawFrame.set(Border.EAST);
		fireDamageEvent(name, new Rectangle(rectangle.x + rectangle.width - 1, rectangle.y + 1, 1, rectangle.height - 2), -1);
	}
}
