...
public class TightEncoding extends AbstractRawEncoding {
	...
	public void processEncodedRect(RFBDisplay display, int x, int y, int width, int height, int encodingType)
		...
		tightNative = rfbModel.getBytesPerPixel() == 4 && pixSize == 3 && rfbModel.getRedMax() == 0xff
	...
	private void doTight(int x, int y, int width, int height, int op) throws IOException {
	...
	private void decodeGradientData(int x, int y, int w, int h, byte[] buf) {
		...
		for (dy = 0; dy < h; dy++) {
			...
			for (dx = 1; dx < w; dx++) {
				...
				rfbModel.getImageBuffer().setRGB(x + dx, y + dy,
	...
	private int readTightColor() throws IOException {
		...
		if (tightNative) {
			return ImageUtil.translate((colorBuff[0] & 0xff) << 16 | (colorBuff[1] & 0xff) << 8 | colorBuff[2] & 0xff,
					rfbModel);
		} else {
